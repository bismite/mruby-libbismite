...
void mrb_init_bi_framebuffer(mrb_state *mrb, struct RClass *bi)
{
  ...
  mrb_define_method(mrb, framebuffer, "initialize", mrb_bi_framebuffer_init, MRB_ARGS_REQ(2)); // w,h
  ...
  mrb_define_method(mrb, framebuffer, "to_texture", mrb_bi_framebuffer_to_texture, MRB_ARGS_NONE());
}
