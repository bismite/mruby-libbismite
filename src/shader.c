...
static mrb_value mrb_shader_initialize(mrb_state *mrb, mrb_value self)
{
    ...
    mrb_get_args(mrb, "iiSS", &w,&h,&vertex_shader,&fragment_shader );
    BiShader* shader = mrb_malloc(mrb,sizeof(BiShader));
    bi_shader_init(shader,w,h, mrb_string_value_cstr(mrb,&vertex_shader), mrb_string_value_cstr(mrb,&fragment_shader) );
...
void mrb_init_bi_shader(mrb_state *mrb,struct RClass *bi)
{
  ...
  mrb_define_method(mrb, shader, "initialize", mrb_shader_initialize, MRB_ARGS_REQ(4)); // w,h,VertexShader,FragmentShader
}
